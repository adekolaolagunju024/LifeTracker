...
- **Calendar** — a month grid of every task by due date (a project filter dropdown narrows it to one), with prev/month/next navigation and a "Today" button. Overdue tasks show in red, completed ones strike through, a day with more than 3 shows a "+N more" that expands in place, clicking a task opens its detail panel, and clicking an empty day opens Add Task with that day pre-filled as the due date

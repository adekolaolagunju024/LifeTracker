...
- **Dashboard** — goal-agnostic on purpose: one combined stats hero (Total/Completed/In Progress plus the overall Goals Progress bar, merged into a single card rather than four separate ones) aggregates every project regardless of type, since Wealth is just one goal among many, not a special headline metric. A distinctly-styled "💬 Chat with AI" card sits right below it — this is the actual landing page after login, so the AI chat entry point lives here, not just a click deeper on All Projects. "Projects at a Glance" shows the first few projects' live stats (tasks, done, an overdue count highlighted in red, progress — a Wealth-type project shows its own £ progress toward its own target instead) as cards, with a "View All Projects →" link rather than a second project-management surface (no view toggle, no "+ New Project" here — that action lives on All Projects and Gantt, its two natural homes). A one-line "⚡ Needs Attention" strip (overdue / high-priority-in-progress counts, or "All caught up 🎉") links straight into Actions rather than duplicating its full task list on the Dashboard too

...
- **Optional card fields (Gantt + Kanban)** — a "🏷️ Fields" toggle in the Gantt/Kanban toolbar lets you choose which extra details show on task rows/cards: Assignee, Priority, Tags, Checklist progress, Cost, and Schedule status. Schedule status is derived, not stored — 🔴 Delayed (past its due date, not done), 🟢 On Track (has a due date, not overdue), or ✅ Done — and also rolls up per-project on the Gantt group header row. Off by default for the denser ones (Priority text, Cost) since priority already shows as a color, and cost isn't relevant to every project — on for the rest, matching how they already looked. Applies to both views at once and remembers your choice

...
- **Create a Project with AI** (optional) — on All Projects, two ways in: upload a `.xlsx`/`.pdf`/`.png`/`.jpg` (a plan, checklist, or spreadsheet) and Claude extracts a project with tasks from it, or — for someone who doesn't know how to break a goal into tasks at all — just describe the goal in plain language (plus an optional target timeframe) and Claude proposes a sequenced set of tasks with realistic dates spread across the available time. Either way lands on the same review screen; every field is editable and nothing is created until you confirm

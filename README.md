...
- **Project sharing & collaboration** — Google-Sheets-style: a project's owner invites another existing account by email (👥 Share, on any project or its sub-folder — sharing always covers the whole tree, and a sub-folder created by a collaborator is still owned by the tree's real owner, not whoever created it) rather than joining a company-wide workspace. Once accepted, the invited member gets full working access — create/edit/delete tasks, edit project details — same as the owner, short of deleting the project or managing who's on it (owner-only). A shared project shows a "👤 Shared by X" badge (on its card and its detail page) so it's always clear whose it is. Tasks can be **assigned** to anyone with access — an initial-avatar badge shows on the task table row, the task detail panel, and now the Gantt row and Kanban card too — and carry a **comment thread** for async updates between collaborators. The Share panel shows each collaborator with a live-ish presence dot (green if active in the last 5 minutes, from a lightweight heartbeat — not a live socket connection). A pending invite shows as a banner + inbox on the invitee's next login, with Accept/Decline

...
- **Create a Project with AI** (optional) — a "💬 Chat with AI" card right on the Dashboard (previously buried a click deeper inside All Projects) opens straight into a real back-and-forth conversation for someone who doesn't know how to break a goal into tasks yet — Claude asks a clarifying question or two (timeframe, scope, current progress) and proposes a project once it has enough, or straight away if the goal's already specific. Uploading a `.xlsx`/`.pdf`/`.png`/`.jpg` instead is still there as a second tab. Either path lands on the same review screen; every field is editable and nothing is created until you confirm. The plain "+ New Project" button remains the fully manual option with no AI involved
